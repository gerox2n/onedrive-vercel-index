...
export function useProtectedSWRInfinite(path: string = '') {
  ...
  function getNextKey(pageIndex: number, previousPageData: OdAPIResponse): (string | null)[] | null {
    ...
    if (pageIndex === 0) return [`/api/?path=${path}&sort=lastModifiedDateTime%20desc`, hashedToken]

    // Add nextPage token to API endpoint
    return [`/api/?path=${path}&next=${previousPageData.next}&sort=lastModifiedDateTime%20desc`, hashedToken]
  }
